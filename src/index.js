...
const runMigrations = async () => {
  ...
  const migrations = [
    ...
    `CREATE TABLE IF NOT EXISTS quote_requests (id INT AUTO_INCREMENT PRIMARY KEY, provider_id INT NOT NULL, package_id INT NULL, user_id INT NULL, client_name VARCHAR(255) NULL, client_email VARCHAR(255) NULL, client_phone VARCHAR(50) NULL, group_id VARCHAR(36) NULL, type_slug VARCHAR(100) NULL, description TEXT NULL, notes TEXT NULL, event_date DATE NULL, guest_count INT NULL, quote_id INT NULL, status ENUM('pending','quoted','cancelled','date_unavailable') NOT NULL DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (provider_id) REFERENCES providers(id) ON DELETE CASCADE)`,
    ...
    `CREATE TABLE IF NOT EXISTS messages (id INT AUTO_INCREMENT PRIMARY KEY, conversation_id INT NOT NULL, sender_id INT NULL, sender_role ENUM('client','admin','visitor') NOT NULL, sender_name VARCHAR(255) NULL, content TEXT NOT NULL, is_read TINYINT(1) DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE CASCADE)`,
    `ALTER TABLE messages ADD COLUMN sender_id INT NULL`,
    `ALTER TABLE messages ADD COLUMN sender_name VARCHAR(255) NULL`,
