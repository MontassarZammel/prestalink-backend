...
const runMigrations = async () => {
  ...
  const migrations = [
    ...
    `CREATE TABLE IF NOT EXISTS provider_types (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL, slug VARCHAR(100) NOT NULL UNIQUE, description TEXT NULL, discount_percentage DECIMAL(5,2) DEFAULT 0, icon VARCHAR(100) NULL, meta_title VARCHAR(255) NULL, meta_description TEXT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS providers (id INT AUTO_INCREMENT PRIMARY KEY, type_id INT NOT NULL, name VARCHAR(255) NOT NULL, slug VARCHAR(255) NOT NULL UNIQUE, short_description VARCHAR(500) NULL, description TEXT NULL, email VARCHAR(255) NULL, phone VARCHAR(50) NULL, city VARCHAR(100) NULL, governorate VARCHAR(100) NULL, address TEXT NULL, price_min DECIMAL(10,2) DEFAULT 0, price_max DECIMAL(10,2) DEFAULT 0, is_featured TINYINT(1) DEFAULT 0, is_active TINYINT(1) DEFAULT 1, rating DECIMAL(3,2) DEFAULT 0, logo VARCHAR(500) NULL, cover_image VARCHAR(500) NULL, standard_fee DECIMAL(10,2) DEFAULT 0, commission_percentage DECIMAL(5,2) DEFAULT 15.00, whatsapp VARCHAR(30) NULL, meta_title VARCHAR(255) NULL, meta_description TEXT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (type_id) REFERENCES provider_types(id) ON DELETE CASCADE)`,
    `CREATE TABLE IF NOT EXISTS provider_images (id INT AUTO_INCREMENT PRIMARY KEY, provider_id INT NOT NULL, image_url VARCHAR(500) NOT NULL, sort_order INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (provider_id) REFERENCES providers(id) ON DELETE CASCADE)`,
    ...
    `CREATE TABLE IF NOT EXISTS quotes (id INT AUTO_INCREMENT PRIMARY KEY, quote_number VARCHAR(50) NULL, client_id INT NULL, provider_id INT NOT NULL, client_name VARCHAR(255) NOT NULL, client_email VARCHAR(255) NOT NULL, client_phone VARCHAR(50) NULL, description TEXT NULL, event_date DATE NULL, price_before_discount DECIMAL(10,2) DEFAULT 0, discount_percentage DECIMAL(5,2) DEFAULT 0, discount_amount DECIMAL(10,2) DEFAULT 0, price_after_discount DECIMAL(10,2) DEFAULT 0, advance_payment DECIMAL(10,2) DEFAULT 0, advance_percentage DECIMAL(5,2) DEFAULT 30, valid_until DATE NULL, status ENUM('draft','sent','accepted','rejected','cancelled') DEFAULT 'draft', payment_status ENUM('pending','partial','paid') DEFAULT 'pending', payment_enabled TINYINT(1) DEFAULT 1, notes TEXT NULL, commission_percentage DECIMAL(5,2) DEFAULT 15.00, group_items TEXT NULL, request_id INT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (provider_id) REFERENCES providers(id) ON DELETE CASCADE)`,
    `CREATE TABLE IF NOT EXISTS quote_requests (id INT AUTO_INCREMENT PRIMARY KEY, provider_id INT NOT NULL, user_id INT NULL, group_id VARCHAR(36) NULL, type_slug VARCHAR(100) NULL, description TEXT NULL, event_date DATE NULL, guest_count INT NULL, status ENUM('pending','quoted','cancelled','date_unavailable') NOT NULL DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (provider_id) REFERENCES providers(id) ON DELETE CASCADE)`,
...
const runSeed = async () => {
  ...
  const seeds = [
    ...
    `INSERT IGNORE INTO users (email, password, full_name, role) VALUES ('admin@mywedding.tn','$2b$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi','Admin MyWedding','admin')`,
